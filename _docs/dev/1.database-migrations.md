...
pnpm --filter @domifa/backend db:dev:migrate-redo
```
...
pnpm --filter @domifa/backend db:dev:generate -- NomDeLaMigration
```
...
```bash
pnpm --filter @domifa/backend db:dev:create -- AjoutTableExample
```

#### Migration automatique (pour les changements d'entités simples)

```bash
pnpm --filter @domifa/backend db:dev:generate -- UpdateEntityExample
```
...
pnpm --filter @domifa/backend db:dev:create -- 2024-01-15-add-user-preferences
