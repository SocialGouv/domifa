...
mkdir -p /mnt/database/transfer
sudo chmod 777 /mnt/database/transfer
```

Récupérer les fichiers en local:

```bash
REMOTE_PROD=domifa-PROD # remplacer par l'alias ou l'ip du serveur

MONGO_DUMP_FROM=/mnt/database/backup-2020-12/backup-2020-12-15-15-47/mongo_mongodump-2020-12-15-15-47.gzip
MONGO_DUMP_TO=/var/tmp/domifa_PROD.mongo.gz

scp ${REMOTE_PROD}:${MONGO_DUMP_FROM} ${MONGO_DUMP_TO}

POSTGRES_DUMP_FROM=/mnt/database/backup-2020-12/backup-2020-12-15-15-47/postgres.pg_dump-2020-12-15-15-47.tar
POSTGRES_DUMP_TO=/var/tmp/domifa_PROD.postgres.dump

scp ${REMOTE_PROD}:${POSTGRES_DUMP_FROM} ${POSTGRES_DUMP_TO}
```
...
Enfin, anonymiser la base de données de preprod:
