...
echo ""
echo "----------------------------------------------------------------------------------------------"
echo "[INFO] CONVERT POSTGRES DB '${POSTGRES_DATABASE}' TABLES to 'UNLOGGED'..."
echo "----------------------------------------------------------------------------------------------"
echo ""
...
(set -x && for tbl in `psql --username "${POSTGRES_USERNAME}" --dbname "${POSTGRES_DATABASE}" -qAt -c "select tablename from pg_tables where schemaname = 'public';"` ; do  psql --username "${POSTGRES_USERNAME}" --dbname "${POSTGRES_DATABASE}" -c "alter table \"$tbl\" SET UNLOGGED" ; done;)
...
echo ""
echo "##############################################################################################"
echo "# [SUCCESS] CONVERT POSTGRES DB '${POSTGRES_DATABASE}' TABLES to 'UNLOGGED': DONE √"
echo "##############################################################################################"
echo ""
