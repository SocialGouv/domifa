...
if [ "$CI" = "true" ]; then
  ...
  (set -x && psql -h "${POSTGRES_HOST}" --username "${POSTGRES_USER}" --dbname "${POSTGRES_DATABASE}" -c "DROP SCHEMA IF EXISTS public")
  if [ $? -ne 0 ]; then
    echo ""
    echo "----------------------------------------------------------------------------------------------"
    echo "[ERROR] UNEXPECTED ERROR RUNNING SCRIPT!"
  ...
  export POSTGRES_PASSWORD=${POSTGRES_PASSWORD}
  (set -x && pg_restore -h "${POSTGRES_HOST}" --username=${POSTGRES_USERNAME} --no-owner --role=${POSTGRES_USERNAME} --exit-on-error --verbose --dbname=${POSTGRES_DATABASE} ${POSTGRES_DUMP_PATH})
  if [ $? -ne 0 ]; then
    echo ""
    echo "----------------------------------------------------------------------------------------------"
    echo "[ERROR] UNEXPECTED ERROR RUNNING SCRIPT!"
