...
import { StructureFilterCriteria, StructureFilterCriteriaSortKey } from '../../utils/structure-filter-criteria'
import { SortValues, StructureType } from '@domifa/common'
...
implements OnInit, OnDestroy, AfterViewInit
{
  ...
  public filters$: BehaviorSubject<StructureFilterCriteria> = new BehaviorSubject(this.filters);
  ...
  public updateFilters<T extends keyof StructureFilterCriteria>({
    ...
    if (element === 'type' &&  this.filters[element] !== value) {
      this.filters = {
        ...this.filters,
  ...
  private getNextSortValue(value: StructureFilterCriteria[keyof StructureFilterCriteria]): SortValues {
  ...
  private applyPaginationFromStore(): void {
    ...
    this.filteredStructures = this.filteredStructures.slice(
      startIndex,
      endIndex
    ) as StructureAdmin[];
  }
