...
DomiFa`;
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    // Suppression des SMS créés par cette migration
    await queryRunner.query(`
      DELETE FROM message_sms
      WHERE "senderName" = 'DomiFa'
        AND "smsId" = 'MON_DOMIFA_IDENTIFIANTS'
        AND "status" = 'TO_SEND'
    `);

    appLogger.warn("Rollback terminé : SMS de création de compte supprimés");
  }
}
