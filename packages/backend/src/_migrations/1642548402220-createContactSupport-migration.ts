  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    if (domifaConfig().envId === "prod" || domifaConfig().envId === "preprod") {
      await queryRunner.query(
        `CREATE TABLE "contact_support" ("uuid" uuid NOT NULL DEFAULT uuid_generate_v4(), "createdAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "version" integer NOT NULL, "userId" integer, "structureId" integer, "content" text NOT NULL, "status" text NOT NULL DEFAULT 'ON_HOLD', "attachement" jsonb,"email" text NOT NULL, "category" text, "name" text NOT NULL, "comments" text, CONSTRAINT "PK_8e4a4781a01061a482fa33e5f5a" PRIMARY KEY ("uuid"))`
      );
      await queryRunner.query(
