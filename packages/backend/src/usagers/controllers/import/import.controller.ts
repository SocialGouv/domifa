...
import { USAGERS_IMPORT_COLUMNS } from "./constants/USAGERS_IMPORT_COLUMNS.const";
...
export class ImportController {
  ...
  public async importExcel(
    ...
    const datas = usagersImportExcelParser.parseFileSync(filePath);
    ...
    for (let rowIndex = 1, len = datas.length; rowIndex < len; rowIndex++) {
      // Ligne
      rowNumber = rowIndex;
      const row: UsagersImportRow = datas[rowIndex];
      ...
      if (errors.length) {
        errorsId = errorsId.concat(errors);
      }
    ...
    if (await this.saveDatas(datas, user)) {
      ...
      throw new HttpException("IMPORT_NOT_COMPLETED", HttpStatus.BAD_REQUEST);
    }
  }
  ...
  private parseRow(
    ...
    rowIndex: number,
    datas: any,
    errorsId: { rowId: string; columnId: number; value: any; label: string }[],
    minDate: Date,
    today: Date,
    nextYear: Date,
    rowNumber: number
    ...
    this.countErrors(notEmpty(row[PRENOM]), rowIndex, PRENOM, datas, errorsId);
  ...
  private async saveDatas(
    ...
    for (let rowIndex = 1, len = datas.length; rowIndex < len; rowIndex++) {
      ...
      // Infos générales
      const usager: Partial<UsagerPG> = this.buildImportUsager(
        row,
        now,
        agent,
        user
  ...
  private buildImportUsager(
    ...
    if (notEmpty(row[DOMICILIATION_EXISTANTE])) {
      entretien.domiciliation = this.convertChoix(row[DOMICILIATION_EXISTANTE]);
    }
  ...
  private countErrors(
    ...
    const position = {
      ...
      label: USAGERS_IMPORT_COLUMNS[idColumn],
    };
