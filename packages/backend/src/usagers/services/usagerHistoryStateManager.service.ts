...
function buildHistoryFromNewDecision({
  ...
  // also add decision to history
  newHistory.decisions.push(decision);
...
async function removeLastDecisionFromHistory({
  ...
  if (newHistory.decisions.length) {
    // also remove decision from history
    newHistory.decisions = newHistory.decisions.filter(
      (d) => d?.uuid !== removedDecisionUUID
    );
  }
